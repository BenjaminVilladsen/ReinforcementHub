...
from config_and_helpers import (
    env, discretize, epsilon_greedy_policy, update_Q, n_bins, state_bounds, alpha_values, gamma_values,
            ...
            for epsilon_decay in epsilon_decay_values:
                ...
                for i_episode in range(n_episodes):
                    ...
                    while True:
                        ...
                        Q = update_Q(Q, state, action, reward, next_state, next_action, alpha, gamma)
                    ...
                    if i_episode % 100 == 0:
                        print(f"Episode: {i_episode + 1}, Average Reward: {np.mean(episode_rewards[-(n_episodes // 10):]):.2f}, Average Length: {np.mean(episode_lengths[-(n_episodes // 10):]):.2f}, Total Reward: {sum(episode_rewards[-(n_episodes // 10):]):.2f}")
